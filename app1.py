...
def evaluate_and_display(name, clf):
    ...
    # Prediction for the selected movie
...
def comparison_graph():
    ...
    for name in algos:
        m = metrics[name]
        # For single movie prediction
        if m["label"] in ["Success", 1]:
    ...
    bars1 = plt.bar(x - width/2, success_rates, width, label="Success %", color="green")
    bars2 = plt.bar(x + width/2, failure_rates, width, label="Failure %", color="red")
    ...
    plt.title(f"Predicted Success/Failure for Movie: {selected_movie}")
    plt.legend()
    plt.tight_layout()
    plt.show()
